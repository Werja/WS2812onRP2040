import neopixel2040 as np
import utime
# the 100 pixel stripe needs 3ms for the ws2812 outputting on pin
# 360 x 3ms = pure 1080ms output time
strip1 = np.strip(100,22,0.1)

start =  utime.ticks_ms()
for i in range(360):
    strip1.fill(np.hue2col(i))
    strip1.show()
print("script runtime",utime.ticks_ms()-start,"ms with outputting")

np.Wait4ThreadEnd()
strip1.clear()

print("core0 puts",np.core0time)
print("core1 puts",np.core1time)

# The pixel manipulation (strip1.fill(np.hue2col(i))) runs on core 0
# while the outputting on core 1 runs
# If more then one stripe is defind, the output is distributed to both cores. 
